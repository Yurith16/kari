// plugins/owner-quitar.js
import { getUser, getEconomy, removeKryons } from '../core/sqlite.js'
...
const RESPUESTAS_OK = [
  'Le quité *{amt}* kryons a *{objetivo}*. Ahora tiene *{total}* kryons en la cartera.',
  'Listo, *{amt}* kryons menos para *{objetivo}*. Total: *{total}* kryons.',
  '*{objetivo}* perdió *{amt}* kryons por decisión mía. Ahora tiene *{total}* kryons en total.',
  'Le bajé *{amt}* kryons a *{objetivo}*, ya tiene *{total}* en total.',
  'Hecho, *{amt}* kryons menos para *{objetivo}*. Total en su cartera: *{total}* kryons.',
  '*{amt}* kryons fuera de la cuenta de *{objetivo}*. Ahora tiene *{total}* kryons.',
]
...
async function quitarKryons(sock, msg, from, targetNum, monto) {
  const targetPerfil = getUser(targetNum)
  const nombre = targetPerfil?.nombre || targetNum

  removeKryons(targetNum, monto)
...
export default {
  ...
  group:       false,
  nsfw:        false,
  descripcion: 'Quita kryons de la cartera de un usuario',
  ...
  async onMessage(sock, msg, { from, text, userNum, groupCfg }) {
    ...
    sesiones.delete(userNum)
    await quitarKryons(sock, msg, from, sesion.targetNum, monto)
  },
  ...
  async execute(sock, msg, { from, sender, args, isGroup, groupCfg }) {
    ...
    if (!target?.num) {
      await sock.sendMessage(from, {
        text: 'Etiqueta a alguien o responde su mensaje para quitarle kryons.'
      }, { quoted: msg })
      return
    }
    ...
    if (!targetPerfil) {
      await sock.sendMessage(from, {
        text: 'Ese usuario no tiene perfil conmigo. No puedo quitarle nada así.'
      }, { quoted: msg })
      return
    }
    ...
    if (!cantidadArg) {
      ...
      await sock.sendMessage(from, {
        text: `¿Cuántos kryons quieres quitarle a *${targetPerfil.nombre}*?`
      }, { quoted: msg })
      return
    }
    ...
    await quitarKryons(sock, msg, from, targetNum, monto)
  }
}