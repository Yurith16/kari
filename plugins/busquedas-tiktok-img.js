...
const DOWNLOAD_DIR = resolve('media/tiktok_img');
...
export default {
  ...
  async execute(sock, msg, { from, args }) {
    if (!args[0]) return sock.sendMessage(from, {
      text: `✦ *Ingresa el tema a buscar.*\nEjemplo: *.ttimg naturaleza*`
    }, { quoted: msg });
    ...
    try {
      ...
      for (const imgUrl of selected) {
        try {
          ...
          const mediaMsg = await sock.generateWAMessage(from, {
            image: buffer,
            caption: enviados === 0 ? `✦ *TikTok Imágenes:* ${query}` : ''
      ...
      console.error('Error TikTok Img:', e.message);
      await sock.sendMessage(from, { react: { text: '❌', key: msg.key } });
    }
  }
};