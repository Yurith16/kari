...
export default {
  ...
  tag: 'ytmp4doc',
  categoria: 'descargas',
  descripcion: 'Descarga videos de YouTube en documento',
  owner: false,
  group: false,

  async execute(sock, msg, { from, args }) {
    if (!args.length) {
      await sock.sendMessage(from, { text: '🌸 ¿Qué video quieres que busque? Dime el nombre o pásame el enlace.' }, { quoted: msg })
      return
    }
  ...
  · · ─────── ·🌸· ─────── · ·
     ${signature}`
      ...
      await sock.sendMessage(from, {
        ...
        mimetype: 'video/mp4',
        fileName: `${(title || 'video').slice(0, 50)}.mp4`
      }, { quoted: msg })
